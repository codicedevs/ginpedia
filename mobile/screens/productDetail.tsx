...
function ProductDetail({
  ...
  const filteredBookmarks = bookmarks.filter(
    (bookmark: Bookmark) => bookmark.productId === Number(productId)
  );
  ...
  const checkInteraction = async () => {
    setIsLiked(
      filteredBookmarks.some(
        (bookmark: Bookmark) => bookmark.type === BookmarkType.WISHLIST
      )
    );
    setIsBookmarked(
      filteredBookmarks.some(
        (bookmark: Bookmark) => bookmark.type === BookmarkType.PURCHASED
      )
    );
  };
  ...
  const handleInteraction = async (option: BookmarkType) => {
    const index = filteredBookmarks.findIndex(
      (bookmark: Bookmark) => bookmark.type === option
    );
    if (index > -1) {
      ...
      <Div bg="background" flex={1}>
        <ScrollDiv showsVerticalScrollIndicator={false} flex={1}>
          <Div h={verticalScale(370)}>
            <Image
              resizeMode="center"
              source={require("../assets/GinBackground.png")}
              h={verticalScale(370)}
              w={"100%"}
            />
          </Div>
          ...
          <Div bg="secondary" px={"xl"}>
            <Div mb={"md"} flexDir="row" justifyContent="space-between">
              ...
              <Div row>
                <TouchableOpacity
                  disabled={isLoading}
                  onPress={() => handleInteraction(BookmarkType.WISHLIST)}
                >
                  <Icon
                    mr={scale(15)}
                ...
                <TouchableOpacity
                  disabled={isLoading}
                  onPress={() => handleInteraction(BookmarkType.PURCHASED)}
                >
                  <Icon
            ...
            <InfoContainer>
              <BoldText color="black">
                Bebidas relacionadas con este producto
              </BoldText>
            ...
            <InfoContainer>
              <BoldText color="black">
                Aderezos perfectos para esta bebida
              </BoldText>
              {product?.combinations &&
                product?.combinations
